...
A small window opens with two steps:
...
**Step 3 — Run the broker** (optional sanity check). Start/Stop buttons and
...
is readable at all, independent of everything above:
...
current (v2, Argon2id) key derivation only, AES-256-CTR content decryption,
and reading a resource's own per-file "individual key". Not yet supported:
legacy (v1, SHA-256) key derivation, RSA private-key decryption / cross-user
resource sharing, NotePod's app-specific inner field cipher (separate from
`solidpod`'s own scheme), and large-file chunking / notification encryption.
See `LessonsLearned.md` and `PLAN.md` for the reasoning if any of these need
adding.
